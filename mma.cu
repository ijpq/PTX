...
__global__ void kernel(int8_t* src_A, int8_t* src_B, void* dst)
{
    ...
    // while loading m8n8 .x1 matrix, thread0~thread7 get 8 rows address of the
    // matrix respectively.
    ...
    asm volatile("ldmatrix.sync.aligned.m8n8.x1.trans.shared::cta.b16 {%0}, [%1];"
                 ...
                 : "r"(col_src_b));
#if DEBUG_BUILD == 1
    if (!threadIdx.x) printf("src B of ldmatrix\n");
    printf("tid: %d, val: %d, %d, %d, %d\n",
           threadIdx.x,
           (int8_t)((r_b)&0xFF),
           (int8_t)((r_b >> 8) & 0xFF),
           (int8_t)((r_b >> 16) & 0xFF),
    ...
    asm volatile("mma.sync.aligned.m8n8k16.row.col.s32.s8.s8.s32 {%0, %1}, {%2}, "
                 "{%3}, {%4, %5};"
                 : "=r"(r_d_0), "=r"(r_d_1)
                 : "r"(r_a), "r"(r_b), "r"(r_c_0), "r"(r_c_1));
// reg -> smem
#if DEBUG_BUILD == 1
    if (!threadIdx.x) printf("result of mma.sync\n");
...
#if DEBUG_BUILD == 1
    if (!threadIdx.x) printf("src A of ldmatrix\n");
    printf("tid: %d, val: %d\n", threadIdx.x, (r_a >> (0 * 8)) & 0xFF);
    printf("tid: %d, val: %d\n", threadIdx.x, (r_a >> (1 * 8)) & 0xFF);
    printf("tid: %d, val: %d\n", threadIdx.x, (r_a >> (2 * 8)) & 0xFF);
    printf("tid: %d, val: %d\n", threadIdx.x, (r_a >> (3 * 8)) & 0xFF);
    __syncthreads();
#endif
...
void get_reference(std::vector<int32_t>& data)
{
    ...
    std::cout << "load reference value from file" << std::endl;
...
int main()
{
    ...
    // cudamalloc
    int8_t *dst_dev_ptr = nullptr, *src_A_dev_ptr = nullptr, *src_B_dev_ptr = nullptr;
    ...
    // cuda memcpy
    cudaMemcpy(
        src_A_dev_ptr, src_A.data(), m8n8k16_src_size, cudaMemcpyKind::cudaMemcpyHostToDevice);
    cudaMemcpy(
        src_B_dev_ptr, src_B.data(), m8n8k16_src_size, cudaMemcpyKind::cudaMemcpyHostToDevice);
    cudaError_t err = cudaGetLastError();
    if (err != cudaError_t::cudaSuccess) printf("err: %s\n", cudaGetErrorString(err));

    // launch kernel
    dim3 grids{1, 1, 1};
    dim3 threads{32, 1, 1};

    kernel<<<grids, threads>>>(src_A_dev_ptr,
                               src_B_dev_ptr,
    ...
    cudaMemcpy(dst.data(),
               dst_dev_ptr,
               m8n8k16_dst_size * sizeof(int32_t),
               cudaMemcpyKind::cudaMemcpyDeviceToHost);
    err = cudaGetLastError();
    if (err != cudaError_t::cudaSuccess) printf("err: %s\n", cudaGetErrorString(err));

    std::cout << "kernel result, dst:" << std::endl;
